...
expose_function("Topic.userRegister",
                "TopicUserRegister", 
                array(
						'username' => array ('type' => 'string'),
						'password' => array ('type' => 'string'),
                 ...
                 'A Register method which register to the elgg',
                 'GET',
false,
false
);

function TopicUserRegister($username, $password, $nickname, $email) {
  $result = FALSE;

  if (empty($username) || empty($password) || empty($nickname) || empty($email))
	return $result;
  else
    return $result = register_user($username, $password, $nickname, $email);
}

expose_function("Topic.login",
                "TopicLogin", 
                array(
						'username' => array ('type' => 'string'),
						'password' => array ('type' => 'string'),
                      ),
                 'A Log in method which log in the current session',
                 'GET',
false,
false
);

function TopicLogin($username, $password) {
  $result = FALSE;

  if (empty($username) || empty($password)) 
	return $result;
  else
    return $result = elgg_authenticate($username, $password);
}

expose_function("Topic.logout",
                "TopicLogout", 
                NULL,
                 'A Log out method which log out the current session',
                 'GET',
                false,
                false
);

function TopicLogout() {
	return $result = logout();
}

expose_function("Topic.getList",
                "TopicGetList", 
                array('group_name' => array('type' => 'string')),
                 'Get the fresh 20 Topicss of the Group (such as Soccer) defined by guid(long)',
                 'GET',
false,
false
);

function TopicGetList($group_name) {

  if (strcmp($group_name,"intl_soccer_to_china") == 0) {
  	$guid = 52;
	...
	foreach ($contents as $content) {
		$arr[$i] = array('time_created' => $content->time_created,
	                'guid' => $content->guid, 
	                'owner_guid' => $content->owner_guid,
	                'object_guid' => $content->object_guid,
	                'type' => $content->type,
	                'title'=> $content->title,
...
expose_function("Topic.postReply",
    "TopicPostReply",
    array( 
          'username' => array ('type' => 'string'),
          'topicId' => array('type' => 'string'),
	      'text' => array ('type' => 'string'),                     
	      ),               
	"Post a reply to a default comment",                
	'GET',                
false,
true
);
...
function TopicPostReply ($username, $topicId, $text){
...
expose_function("Topic.getReply",
    "TopicGetReply",
array(
          'topicId' => array('type' => 'string'), ),
	"retrive replys from the certain topic",                
	'GET',                
false,
false
);
...
function TopicGetReply ($topicId){
	$group = get_entity($topicId);	
	$options = array( 'guid' => $topicId,		
	...
	$i = 0;
	foreach ($contents as $content) {
		$user = get_entity((int) $content->owner_guid);
		$arr[$i] = array('time_created' => $content->time_created,
		                'comment_id' => $content->id,
		                'topic_id' => $content->entity_guid,  //topic Id
		                'owner'=> $user->name,
		                'type' => $contetn->value_type,
...
expose_function("Topic.addNew",            
	"TopicAddNew",                
	array( 'username' => array ('type' => 'string'),  
           'groupId' => array ('type' => 'string'),
	       'text' => array ('type' => 'string'),                     ),               
	"Post a status update to a certain topic",                
	'GET',                
	false,                
	false
);
...
function TopicAddNew($username, $groupId, $text) {
	...
	$comment->access_id = 0; //ACCESS_PUBLIC;
...
// For Swiming events only

expose_function("Event.getInfo",
    "EventGetInfo",
array(
    'Id' => array('type' => 'string'), ),
	"retrive info from the certain events",                
	'GET',                
false,
false
);
...
function EventGetInfo ($Id){
	
	$topic = get_entity((int) $Id);
	if (!$topic) {
		throw new InvalidParameterException("Bad Topic Id");
		return false;
	}
	
	$pos = strPos($topic->description, "currentEvent:");
	$currentSubInfo = substr($topic->description, $pos+13, 17);
	$token = strtok($currentSubInfo, ".");
	$i=0;
	while ($token!=false){
		$currentSubInfo[$i++] = $token;
		$token = strtok(" ");
	}
	
	$pos = strPos($topic->description, "regEvent:");
	$regSubInfo = substr($topic->description, $pos+9, 3);
	$token = strtok($regSubInfo, ".");
	$i=0;
	while ($token!=false){
		$regSubInfo[$i++] = $token;
		$token = strtok(" ");
	...
	$arr = array(           'event_id' => $topic->guid,  
			                'paraller' => $currentSubInfo[0],
							'e11' => $currentSubInfo[2],
							'e12' => $currentSubInfo[4],
							'h11' => $currentSubInfo[6],
							'h12' => $currentSubInfo[8],
							'e21' => $currentSubInfo[10],
							'e22' => $currentSubInfo[12],
							'h21' => $currentSubInfo[14],
							'h22' => $currentSubInfo[16],
							'e31' => $regSubInfo[0],
							'e32' => $regSubInfo[2]
	);
	$json = json_encode($arr);
...
/*
expose_function("Event.postCurrentInfo",
    "EventPostCurrentInfo",
	array( 
		'Id' => array('type' => 'string'),
        'event123' => array ('type' => 'string'),          ),            
		"post info to the certain events",                
	    'GET',                
false,
false
);
...
function EventPostCurrentInfo($Id, $event12){  //currentEvent:paraller.$e11.$e12.$h11.$h12.$e21.$e22.$h21.$h22
	$topic = new ElggObject($Id);
	if (!$topic) {
		throw new InvalidParameterException("Bad Event Id");
		return "false";
	}
	// check text
	if (empty($event12)) {
	...
	if($pos > 0)
	  $newDesc = substr_replace($topic->description, $event12, $pos, $len);
	else 
	  $newDesc = $topic->description.$event12;
	...
	$arr = array(         
	 ...
	 'container_guid' => $topic->container_guid,
	 'result' => $result
	);
	$json = json_encode($arr);
...
expose_function("Event.postRegInfo",
    "EventPostRegInfo",
array(
		'Id' => array('type' => 'string'),
        'event3' => array ('type' => 'string'),          ),            
		"post info to the certain events",                
	    'GET',                
false,
false
...
function EventPostRegInfo($Id, $event3){  // regEvent:01.23
	...
	$len = strlen($event3);
	if($pos > 0)
	  $newDesc = substr_replace($topic->description, $event3, $pos, $len);
	else 
	  $newDesc = $topic->description.$event3;
